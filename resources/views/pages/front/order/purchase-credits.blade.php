                ...
                @foreach ($plans as $plan)
                    <div class="border-foreground/20 bg-neutral overflow-hidden rounded-lg border shadow-sm transition-all hover:shadow-md">
                        <div class="p-6">
                            ...
                            <div class="mt-4">
                                @php
                                    $displayCurrencyCode = $plan->display_currency ?? $plan->currency;
                                    $displayAmount = $plan->display_price ?? (float) $plan->price;
                                @endphp
                                <p class="text-foreground text-3xl font-bold">
                                    {{ Number::currency($displayAmount, $displayCurrencyCode, $locale) }}
                                </p>
