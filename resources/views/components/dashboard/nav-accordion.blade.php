...
<li class="hs-accordion{{ $hasActiveChild ? ' active' : '' }}" id="{{ $id }}">
    <button class="hs-accordion-toggle focus:outline-hidden text-foreground hover:bg-foreground/20 flex w-full items-center gap-x-2.5 rounded-lg px-2 py-1 text-start text-sm" type="button" aria-expanded="{{ $hasActiveChild ? 'true' : 'false' }}" aria-controls="{{ $id }}-child">
        @if ($icon)
